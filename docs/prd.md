...
- **FR2:** The world comprises strata over a procedurally generated terrain heightmap with water bodies; entities have continuous `(x, y, z)` positions and a stratum attribute. **Descoped (implemented state):** the world evolved into a cube-sphere planet with **two** active strata — `SURFACE` and `SKY`. The underground layer was cut: it was hard to see and the governor over-fixated on it, and "going to ground" is now a per-entity `hide()`/burrow *state* rather than a layer. The engine's stratum machinery is still generic (the `UNDERGROUND=0` enum value survives internally), but no live species uses it and the plugin API exposes only `SURFACE`/`SKY`. Other mentions of three strata below are historical.
